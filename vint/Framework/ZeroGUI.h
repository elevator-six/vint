wchar_t* s2wc(const const char* c)
{
...
void Combobox(const char* name, FVector2D size, int* value, const const char* arg, ...)
{
	elements_count++;
	...
	va_list arguments;
	for (va_start(arguments, arg); arg != NULL; arg = va_arg(arguments, const const char*))
	{
		//Selected Element
		if (num == *value)
		{
			FVector2D _textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter((const char*)arg, _textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
		}
		...
		if (checkbox_enabled[elements_count])
		{
			...
			PostRenderer::TextLeft((const char*)arg, FVector2D{ element_pos.X + 5.0f, element_pos.Y + 15.0f }, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
		}
		num++;
	}
	va_end(arguments);
...
void Hotkey(const char* name, FVector2D size, int* key)
{
	...
	else
	{
		//Text
		FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
		TextCenter((const char*)VirtualKeyCodeToString(*key).c_str(), textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
...
void Render()
{
	for (int i = 0; i < 128; i++)
	{
		if (PostRenderer::drawlist[i].type != -1)
		{
			...
			{
				TextLeft(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
			}
			//TextCenter
			else if (PostRenderer::drawlist[i].type == 3)
			{
				TextCenter(PostRenderer::drawlist[i].name, PostRenderer::drawlist[i].pos, PostRenderer::drawlist[i].color, PostRenderer::drawlist[i].outline);
			}
			//Draw_Line
...
void DrawRect(const FVector2D& min, const FVector2D& max, FLinearColor color, float thickness)
{
...
void FilledRect(float x, float y, float w, float h, FLinearColor color)
{
	drawFilledRect(FVector2D{ x, y }, w, h, color);
}
...
{
	FilledRect(x + borderPx, y, w, borderPx, color);
	FilledRect(x + w - w + borderPx, y, w, borderPx, color);
	FilledRect(x, y, borderPx, h, color);
	FilledRect(x, y + h - h + borderPx * 2, borderPx, h, color);
	FilledRect(x + borderPx, y + h + borderPx, w, borderPx, color);
	FilledRect(x + w - w + borderPx, y + h + borderPx, w, borderPx, color);
	FilledRect(x + w + borderPx, y, borderPx, h, color);
...
auto DrawHealthBar(FVector RootPosition, float Width, float Height, float Health, float RelativeDistance)-> void
{
	...
	int R = 255 - G;
	FLinearColor bar_color{ (float)R / 255.0f, (float)G / 255.0f, 0, 1 };
...
{
	int G = (255 * shield / 100);
	int R = 255 - G;
	FLinearColor bar_color{ (float)R / 255.0f, (float)G / 255.0f, 0, 1 };

	auto HPBoxWidth = 1 / RelativeDistance;

	auto HPBox_X = RootPosition.X - Width / 2 - 9 - HPBoxWidth;
	auto HPBox_Y = RootPosition.Y - Height / 2 + (Height - Height * (shield / 100));
